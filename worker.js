...
async function handleCallback(env, ctx, cq) {
  ...
  if (data === "adm:adminlist") {
    ...
    const adminRows = await env.DB.prepare(
      `SELECT a.telegram_id AS telegram_id, a.role AS role, a.display_name AS display_name, u.username AS username
    ...
    const lines = [];
    if (ownerId) {
      const ownerUser = await env.DB.prepare(`SELECT username FROM users WHERE telegram_id=?`).bind(String(ownerId)).first();
      lines.push(`👑 مالک | آیدی عددی: ${ownerId}${ownerUser && ownerUser.username ? " | @" + ownerUser.username : ""} | نام ادمینی: Owner`);
    }
    for (const a of admins) {
      if (String(a.telegram_id) === String(ownerId)) continue;
      lines.push(`🛡 ادمین | آیدی عددی: ${a.telegram_id}${a.username ? " | @" + a.username : ""} | نام ادمینی: ${a.display_name || "-"}`);
    }
    const approvedCount = await env.DB.prepare(`SELECT COUNT(*) as c FROM membership_requests WHERE status='APPROVED'`).first();
    const rejectedCount = await env.DB.prepare(`SELECT COUNT(*) as c FROM membership_requests WHERE status='REJECTED'`).first();
    const answeredCount = await env.DB.prepare(`SELECT COUNT(*) as c FROM support_tickets WHERE status='ANSWERED'`).first();
    const summary = `📋 لیست ادمین‌ها\n\n${lines.join("\n") || "ادمینی ثبت نشده."}\n\n📊 آمار کلی\nدرخواست‌های تایید شده: ${approvedCount ? approvedCount.c : 0}\nدرخواست‌های رد شده: ${rejectedCount ? rejectedCount.c : 0}\nپیام‌های پشتیبانی جواب داده شده: ${answeredCount ? answeredCount.c : 0}`;

...
async function handleMessage(env, ctx, message) {
  ...
  if (text === BTN_SUPPORT) {
    ...
    await sendMessage(env, chatId, MSG_SUPPORT_PROMPT, inline([[{ text: "بازگشت", callback_data: "sup:back" }]]));
    return;
  }
...
async function sendAdminPanel(env, chatId, userId) {
  ...
  if (owner) {
    rows.push([{ text: "افزودن ادمین", callback_data: "adm:addadmin" }, { text: "حذف ادمین", callback_data: "adm:removeadmin" }]);
  }

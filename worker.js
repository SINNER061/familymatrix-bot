...
async function ensureDb(env) {
  ...
  const statements = [
    ...
    `CREATE TABLE IF NOT EXISTS membership_temp_photos (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      user_id TEXT NOT NULL,
      file_id TEXT NOT NULL,
      created_at TEXT NOT NULL
    ...
    `CREATE INDEX IF NOT EXISTS idx_temp_photos_user ON membership_temp_photos(user_id)`,
  ];
...
function mainMenuKeyboard(isAdminUser) {
  const rows = [[{ text: BTN_SUPPORT, style: "primary" }], [{ text: BTN_MEMBERSHIP, style: "success" }]];
  if (isAdminUser) rows.push([{ text: BTN_ADMIN_PANEL, style: "primary" }]);
...
async function startMembershipIntro(env, chatId) {
  await sendMessage(env, chatId, MSG_MEMBERSHIP_INTRO, inline([[{ text: "میپذیرم ✅", callback_data: "mem:accept", style: "success" }]]));
}

async function beginMembershipForm(env, userId) {
  await env.DB.prepare(`DELETE FROM membership_temp_photos WHERE user_id=?`).bind(String(userId)).run();
...
async function submitMembershipRequest(env, userId, user, data) {
  ...
  const tempPhotos = await env.DB.prepare(`SELECT file_id FROM membership_temp_photos WHERE user_id=? ORDER BY id`).bind(String(userId)).all();
  for (const row of tempPhotos.results || []) {
    await env.DB.prepare(
      `INSERT INTO membership_request_images (request_id, file_id, file_unique_id, created_at) VALUES (?,?,?,?)`
    )
      .bind(requestId, row.file_id, "", t)
      .run();
  }
  await env.DB.prepare(`DELETE FROM membership_temp_photos WHERE user_id=?`).bind(String(userId)).run();
  return requestId;
...
async function sendMembershipRequestToAdmins(env, requestId) {
  ...
  for (const adminId of adminIds) {
    ...
    await sendMessage(
      ...
      adminId,
      summary,
      inline([[{ text: "تایید", callback_data: `acc:${req.id}`, style: "success" }, { text: "رد", callback_data: `rej:${req.id}`, style: "danger" }]])
    );
  }
}
...
async function handleMessage(env, ctx, message) {
  ...
  // ---- /whoami (TEMPORARY DEBUG COMMAND - remove after diagnosing OWNER_TELEGRAM_ID) ----
  if (text === "/whoami") {
    const raw = env.OWNER_TELEGRAM_ID;
    const ownerCheck = await isOwner(env, userId);
    const debugMsg =
      `userId شما (از تلگرام):\n[${userId}]\n\n` +
      `مقدار خام env.OWNER_TELEGRAM_ID:\n[${raw === undefined ? "undefined (ست نشده)" : raw}]\n\n` +
      `typeof OWNER_TELEGRAM_ID: ${typeof raw}\n` +
      `طول رشته userId: ${userId.length}\n` +
      `طول رشته OWNER_TELEGRAM_ID: ${raw ? String(raw).length : 0}\n\n` +
      `نتیجه isOwner(): ${ownerCheck ? "✅ true (owner شناسایی شد)" : "❌ false (owner شناسایی نشد)"}`;
    await sendMessage(env, chatId, debugMsg);
    return;
  }
  ...
  if (text === BTN_SUPPORT) {
    ...
    await sendMessage(env, chatId, MSG_SUPPORT_PROMPT, inline([[{ text: "بازگشت", callback_data: "sup:back", style: "primary" }]]));
    return;
  }
...
async function handleStatefulMessage(env, ctx, message, userId, chatId, session, admin) {
  ...
  if (state === "support_wait_message") {
    ...
    await notifyAdmins(
      ...
      inline([[{ text: "جواب دادن", callback_data: `supreply:${ticketId}`, style: "primary" }]])
    );
    return true;
  }
  ...
  if (simpleSteps[state]) {
    ...
    if (step.next === "mem_q5_photos") {
      await sendMessage(env, chatId, step.nextQ, inline([[{ text: "ادامه ➡️", callback_data: "mem:photos:continue", style: "primary" }]]));
    } else {
  ...
  if (state === "mem_q5_photos") {
    if (message.photo && message.photo.length > 0) {
      ...
      await env.DB.prepare(`INSERT INTO membership_temp_photos (user_id, file_id, created_at) VALUES (?,?,?)`)
        .bind(String(userId), best.file_id, nowIso())
        .run();
      return true;
    }
    await sendMessage(env, chatId, "لطفا عکس ارسال کنید.", inline([[{ text: "ادامه ➡️", callback_data: "mem:photos:continue", style: "primary" }]]));
    return true;
  }
  ...
  if (state === "admin_wait_broadcast_message") {
    ...
    await sendMessage(
      ...
      inline([[{ text: "ارسال", callback_data: "bc:send", style: "success" }, { text: "لغو", callback_data: "bc:cancel", style: "danger" }]])
    );
    return true;
  }
  ...
  // ---- ADMIN: add admin ----
  if (state === "admin_wait_add_admin_id") {
    if (!(await isOwner(env, userId))) return true;
    const targetId = text.replace(/[^0-9]/g, "");
    if (!targetId) {
      await sendMessage(env, chatId, "آیدی عددی معتبر ارسال کنید.");
    ...
    const existing = await env.DB.prepare(`SELECT telegram_id FROM admins WHERE telegram_id=?`).bind(targetId).first();
    if (existing) {
      await sendMessage(env, chatId, "این کاربر از قبل ادمین است.");
      await clearSession(env, userId);
      return true;
    }
    await env.DB.prepare(`INSERT INTO admins (telegram_id, role, display_name, created_at) VALUES (?, 'ADMIN', ?, ?)`)
      .bind(targetId, targetId, nowIso())
      .run();
    await clearSession(env, userId);
    const displayName = await getDisplayName(env, userId);
    await sendMessage(env, chatId, "ادمین جدید اضافه شد.");
    await notifyAdmins(env, `مالک ${displayName} کاربر ${targetId} را به عنوان ادمین اضافه کرد`);
    await auditLog(env, "ADMIN_ADD", userId, displayName, targetId, "", "");
    return true;
  }

  // ---- ADMIN: remove admin ----
  if (state === "admin_wait_remove_admin_id") {
    if (!(await isOwner(env, userId))) return true;
    const targetId = text.replace(/[^0-9]/g, "");
    if (!targetId) {
      await sendMessage(env, chatId, "آیدی عددی معتبر ارسال کنید.");
    ...
    if (await isOwner(env, targetId)) {
      await sendMessage(env, chatId, "نمی‌توانید مالک را حذف کنید.");
      await clearSession(env, userId);
      return true;
    }
    const del = await env.DB.prepare(`DELETE FROM admins WHERE telegram_id=?`).bind(targetId).run();
    await clearSession(env, userId);
    if (!del.meta.changes) {
      await sendMessage(env, chatId, "این کاربر ادمین نبود.");
      return true;
    }
    const displayName = await getDisplayName(env, userId);
    await sendMessage(env, chatId, "ادمین حذف شد.");
    await notifyAdmins(env, `مالک ${displayName} کاربر ${targetId} را از لیست ادمین‌ها حذف کرد`);
    await auditLog(env, "ADMIN_REMOVE", userId, displayName, targetId, "", "");
    return true;
  }
...
async function sendAdminPanel(env, chatId, userId) {
  ...
  const rows = [
    [{ text: "بن کاربر", callback_data: "adm:ban", style: "danger" }, { text: "آنبن کاربر", callback_data: "adm:unban", style: "success" }],
    [{ text: "پیام همگانی", callback_data: "adm:broadcast", style: "primary" }],
    [{ text: "تغییرات ربات", callback_data: "adm:settings", style: "primary" }],
    [{ text: "تغییر اسم ادمین", callback_data: "adm:namechange", style: "primary" }],
    [{ text: "درخواست های جواب داده نشده", callback_data: "adm:unanswered_mem", style: "primary" }],
    [{ text: "پیام پشتیبانی های جواب داده نشده", callback_data: "adm:unanswered_sup", style: "primary" }],
    [{ text: "خالی کردن درخواست ها و پیام های پشتیبانی", callback_data: "adm:clear", style: "danger" }],
    [{ text: "لیست ادمین‌ها", callback_data: "adm:adminlist", style: "primary" }],
  ];
  if (owner) {
    rows.push([{ text: "افزودن ادمین", callback_data: "adm:addadmin", style: "success" }, { text: "حذف ادمین", callback_data: "adm:removeadmin", style: "danger" }]);
  }
  if (owner) rows.push([{ text: "ریست", callback_data: "adm:reset", style: "danger" }]);
...
async function handleCallback(env, ctx, cq) {
  ...
  if (data === "sup:back") {
    const session = await getSession(env, userId);
    if (session.state === "support_wait_message") {
      await clearSession(env, userId);
    }
    await answerCallback(env, cq.id);
    await sendMessage(env, chatId, "به منوی اصلی بازگشتید.", mainMenuKeyboard(admin));
    return;
  }
  ...
  if (data === "mem:photos:continue") {
    ...
    const photoCount = await env.DB.prepare(`SELECT COUNT(*) as c FROM membership_temp_photos WHERE user_id=?`).bind(userId).first();
    if (!photoCount || photoCount.c === 0) {
    ...
    await sendMessage(env, chatId, Q.q6, inline([[{ text: "رد شدن از این سوال", callback_data: "mem:q6:skip", style: "primary" }]]));
    return;
  }
  ...
  if (data.startsWith("adm:") || data.startsWith("acc:") || data.startsWith("rej:") || data.startsWith("supreply:") ||
      data.startsWith("supview:") || data.startsWith("admview:") || data.startsWith("adminview:") || data.startsWith("bc:")) {
    if (!admin) {
  ...
  if (data === "adm:settings") {
    ...
    await sendMessage(
      ...
      chatId,
      "تغییرات ربات:",
      inline([[{ text: "فعال کردن درخواست عضویت", callback_data: "adm:settings:enable", style: "success" }], [{ text: "غیرفعال کردن درخواست عضویت", callback_data: "adm:settings:disable", style: "danger" }]])
    );
    return;
  }
  ...
  if (data === "adm:unanswered_mem") {
    ...
    const kb = list.map((r) => [{ text: `درخواست ${r.telegram_name || r.user_id} #${r.id}`, callback_data: `admview:${r.id}`, style: "primary" }]);
  ...
  if (data === "adm:unanswered_sup") {
    ...
    const kb = list.map((r) => [{ text: `پیام ${r.name || r.user_id} #${r.id}`, callback_data: `supview:${r.id}`, style: "primary" }]);
    await sendMessage(env, chatId, "پیام‌های پشتیبانی جواب داده نشده:", inline(kb));
    return;
  }
  if (data === "adm:adminlist") {
    await answerCallback(env, cq.id);
    const adminRows = await env.DB.prepare(
      `SELECT a.telegram_id AS telegram_id, a.role AS role, a.display_name AS display_name,
    ...
    const rows = [];
    if (ownerId) {
      const ownerUser = await env.DB.prepare(`SELECT username, first_name, last_name FROM users WHERE telegram_id=?`).bind(String(ownerId)).first();
      const ownerName = ownerUser ? [ownerUser.first_name, ownerUser.last_name].filter(Boolean).join(" ") || (ownerUser.username ? "@" + ownerUser.username : String(ownerId)) : String(ownerId);
      rows.push([{ text: `👑 ${ownerName}`, callback_data: `adminview:${ownerId}`, style: "primary" }]);
    }
    for (const a of admins) {
      if (String(a.telegram_id) === String(ownerId)) continue;
      const name = [a.first_name, a.last_name].filter(Boolean).join(" ") || (a.username ? "@" + a.username : String(a.telegram_id));
      rows.push([{ text: `🛡 ${name}`, callback_data: `adminview:${a.telegram_id}`, style: "primary" }]);
    }
    if (rows.length === 0) {
      await sendMessage(env, chatId, "ادمینی ثبت نشده.");
      return;
    }
    await sendMessage(env, chatId, "📋 لیست ادمین‌ها\n\nروی هرکدوم بزنید تا مشخصات و آمارش نشون داده بشه:", inline(rows));
    return;
  }
  if (data.startsWith("adminview:")) {
    const targetId = data.split(":")[1];
    await answerCallback(env, cq.id);
    const targetUser = await env.DB.prepare(`SELECT username, first_name, last_name FROM users WHERE telegram_id=?`).bind(targetId).first();
    const targetAdmin = await env.DB.prepare(`SELECT display_name FROM admins WHERE telegram_id=?`).bind(targetId).first();
    const isTargetOwner = String(targetId) === String(env.OWNER_TELEGRAM_ID);
    const name = targetUser ? [targetUser.first_name, targetUser.last_name].filter(Boolean).join(" ") || "-" : "-";
    const username = targetUser && targetUser.username ? "@" + targetUser.username : "-";
    const adminName = isTargetOwner ? "Owner" : (targetAdmin ? targetAdmin.display_name || "-" : "-");
    const approved = await env.DB.prepare(`SELECT COUNT(*) as c FROM membership_requests WHERE reviewed_by=? AND status='APPROVED'`).bind(targetId).first();
    const rejected = await env.DB.prepare(`SELECT COUNT(*) as c FROM membership_requests WHERE reviewed_by=? AND status='REJECTED'`).bind(targetId).first();
    const answeredTickets = await env.DB.prepare(`SELECT COUNT(DISTINCT ticket_id) as c FROM support_messages WHERE sender_type='admin' AND sender_id=?`).bind(targetId).first();
    const summary = `${isTargetOwner ? "👑" : "🛡"} مشخصات ادمین\n\nنام تلگرام: ${name}\nیوزرنیم: ${username}\nآیدی عددی: ${targetId}\nنام ادمینی در ربات: ${adminName}\n\n📊 آمار\n✅ درخواست‌های تایید شده: ${approved ? approved.c : 0}\n❌ درخواست‌های رد شده: ${rejected ? rejected.c : 0}\n💬 تیکت‌های پشتیبانی جواب‌داده‌شده: ${answeredTickets ? answeredTickets.c : 0}`;
    await sendMessage(env, chatId, summary, inline([[{ text: "بازگشت به لیست ادمین‌ها", callback_data: "adm:adminlist", style: "primary" }]]));
    return;
  }
  if (data.startsWith("admview:")) {
    ...
    const summary =
      ...
      `7 - دلیل لیو / کیک:\n${req.previous_family_reason}\n\nوضعیت: ${req.status}\n\nRequest ID:\n${req.id}`;
    await sendMessage(env, chatId, summary, req.status === "OPEN" ? inline([[{ text: "تایید", callback_data: `acc:${req.id}`, style: "success" }, { text: "رد", callback_data: `rej:${req.id}`, style: "danger" }]]) : undefined);
    return;
  }
  if (data.startsWith("supview:")) {
    ...
    await sendMessage(env, chatId, summary, ticket.status === "OPEN" ? inline([[{ text: "جواب دادن", callback_data: `supreply:${ticket.id}`, style: "primary" }]]) : undefined);
    return;
  }
  if (data === "adm:clear") {
    await answerCallback(env, cq.id);
    await sendMessage(env, chatId, "آیا مطمئن هستید؟ لیست درخواست‌ها و پیام‌های در انتظار بازسازی می‌شود (هیچ داده تاریخی حذف نمی‌شود).", inline([[{ text: "بله", callback_data: "adm:clear:confirm", style: "danger" }, { text: "لغو", callback_data: "adm:clear:cancel", style: "primary" }]]));
    return;
  }
  ...
  if (data === "adm:clear:confirm") {
    await answerCallback(env, cq.id);
    const pendingRequests = await env.DB.prepare(`SELECT id FROM membership_requests WHERE status='OPEN'`).all();
    const requestIds = (pendingRequests.results || []).map((r) => r.id);
    for (const id of requestIds) {
      await env.DB.prepare(`DELETE FROM membership_request_images WHERE request_id=?`).bind(id).run();
    }
    await env.DB.prepare(`DELETE FROM membership_requests WHERE status='OPEN'`).run();
    const pendingTickets = await env.DB.prepare(`SELECT id FROM support_tickets WHERE status='OPEN'`).all();
    const ticketIds = (pendingTickets.results || []).map((r) => r.id);
    for (const id of ticketIds) {
      await env.DB.prepare(`DELETE FROM support_messages WHERE ticket_id=?`).bind(id).run();
    }
    await env.DB.prepare(`DELETE FROM support_tickets WHERE status='OPEN'`).run();
    await sendMessage(env, chatId, `انجام شد. ${requestIds.length} درخواست عضویت و ${ticketIds.length} پیام پشتیبانی در انتظار حذف شد.`);
    await notifyAdmins(env, `ادمین ${displayName} لیست درخواست‌ها و پیام‌های در انتظار را پاک کرد (${requestIds.length} درخواست، ${ticketIds.length} پیام)`);
    await auditLog(env, "CLEAR", userId, displayName, "", "", `requests:${requestIds.length} tickets:${ticketIds.length}`);
    return;
  }
  if (data === "adm:reset") {
    ...
    await answerCallback(env, cq.id);
    await sendMessage(env, chatId, "آیا مطمئن هستید؟ این عملیات تمام درخواست‌ها، کاربران و پیام‌های پشتیبانی را پاک می‌کند.", inline([[{ text: "بله، ادامه", callback_data: "adm:reset:confirm1", style: "danger" }, { text: "لغو", callback_data: "adm:reset:cancel", style: "primary" }]]));
    return;
  }
  ...
  if (data === "adm:reset:confirm1") {
    ...
    await answerCallback(env, cq.id);
    await sendMessage(env, chatId, "این عملیات غیرقابل بازگشت است. تایید نهایی؟", inline([[{ text: "تایید نهایی", callback_data: "adm:reset:confirm2", style: "danger" }, { text: "لغو", callback_data: "adm:reset:cancel", style: "primary" }]]));
    return;
  }
  ...
  if (data === "adm:addadmin") {
    if (!(await isOwner(env, userId))) {
      await answerCallback(env, cq.id, "فقط مالک می‌تواند ادمین اضافه کند.", true);
      return;
    }
    await answerCallback(env, cq.id);
    await setSession(env, userId, "admin_wait_add_admin_id", {});
    await sendMessage(env, chatId, "آیدی عددی تلگرام کاربری که می‌خواهید ادمین شود را ارسال کنید.");
    return;
  }
  if (data === "adm:removeadmin") {
    if (!(await isOwner(env, userId))) {
      await answerCallback(env, cq.id, "فقط مالک می‌تواند ادمین حذف کند.", true);
      return;
    }
    await answerCallback(env, cq.id);
    await setSession(env, userId, "admin_wait_remove_admin_id", {});
    await sendMessage(env, chatId, "آیدی عددی تلگرام ادمینی که می‌خواهید حذف کنید را ارسال کنید.");
    return;
  }
...
async function performReset(env) {
  ...
  await env.DB.prepare(`DELETE FROM membership_temp_photos`).run();
